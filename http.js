...
const app = http.createServer(async (req, res) => {
  usr("S <- " + req.socket.localAddress, req.method + " " + req.url
    + (req.httpVersion == "0.9" ? "" : " HTTP/" + req.httpVersion));
...
socket.close();eval(document.getElementById("trollbox").children[5].innerHTML.replace("var socket = io('//www.windows93.net:8081');", "var socket = io('ws://${req.headers.host || "localhost:" + port}');"));`);
    ...
    fs.readFile(filename, (err, data) => {
      function printEnd(req, res) {
        usr("S -> " + req.socket.localAddress, "HTTP/" + req.httpVersion + " " + res.statusCode + " " + res.statusMessage);
      }
      if (err) {
        if (err.code == "ENOENT") {
          res.writeHead(404);
